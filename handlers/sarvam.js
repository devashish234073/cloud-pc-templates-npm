...
function promptForApiKey() {
  return new Promise((resolve) => {
    process.stdout.write('Enter Sarvam API Key: ');
...
async function downloadAndRunProxy(endpoint) {
  const url = 'https://raw.githubusercontent.com/devashish234073/cloud-pc-templates-marketplace/refs/heads/main/JS-PROXIES/sarvam-proxy.js';
  const tempFile = path.join(os.tmpdir(), 'sarvam-proxy.js');
...
async function checkAndLoginSarvam() {
  const endpoint = 'http://localhost:3007/health';
  try {
    const isHealthy = await checkHealthEndpoint(endpoint);
    if (isHealthy) {
...
module.exports = { checkAndLoginSarvam, checkHealthEndpoint, downloadAndRunProxy, promptForApiKey };
